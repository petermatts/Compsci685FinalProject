...
def parse_args():
    ...
    parser.add_argument('--output_gold_csv', type=str, required=True, help='Path to output processed')
...
def process_csv(csv_path: str, gold_path:str) -> str:
    df = pd.read_csv(csv_path)
    df.rename(columns={'answer': 'gold'}, inplace=True)
    df.rename(columns={'model_response': 'answer'}, inplace=True)
    df['gold'] = df['gold'].apply(lambda x: '$'+str(x)+'$')
    file_name = os.path.basename(csv_path)
    save_name=os.path.join(gold_path, f"gold_{file_name}")
    df.to_csv(save_name)
    return save_name
...
def process_answers(df: pd.DataFrame, gold_is_latex: bool) -> pd.DataFrame:
    ...
    # print("ct",ct)
    # print("df", len(results_df))
    # Add summary stats to the dataframe 
   
    accuracy_by_level = results_df.groupby('level')['is_correct'].agg(['sum', 'count'])
    accuracy_by_level['accuracy'] = accuracy_by_level['sum'] / accuracy_by_level['count']
    results_df.attrs['accuracy_by_level'] = accuracy_by_level['accuracy'].to_dict()
...
def main():
    ...
    for file in os.listdir(args.input_csv):
        if file.endswith(".csv"):
            print(f"{file}:")
            csv_input_path=os.path.join(args.input_csv,file)
            op_path= os.path.join(args.output_csv, f"op_{file}")
            save_name=process_csv(csv_input_path,args.output_gold_csv)
            input_df= load_csv_data(save_name)

            
            # Process answers and extract sympy objects
            results_df = process_answers(input_df, True)
